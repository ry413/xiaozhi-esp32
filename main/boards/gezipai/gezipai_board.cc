...
#define INFRARED_SENSOR_POLL_INTERVAL_US (200 * 1000)
#define INFRARED_SENSOR_DEBOUNCE_COUNT 3
#define INFRARED_SENSOR_ACTIVE_LEVEL 1
#define INFRARED_SENSOR_ABSENCE_TIMEOUT_US (60 * 1000 * 1000)
#define INFRARED_SENSOR_PRESENT_RETRY_US (5 * 1000 * 1000)
...
private:
    ...
    bool led_on_ = false;
    esp_timer_handle_t infrared_timer_ = nullptr;
    int infrared_stable_level_[2] = {-1, -1};
    int infrared_candidate_level_[2] = {-1, -1};
    int infrared_candidate_count_[2] = {0, 0};
    bool infrared_pin_active_[2] = {false, false};
    bool infrared_customer_present_ = false;
    int64_t infrared_last_active_time_ = 0;
    int64_t infrared_last_notify_time_ = 0;
    ...
    static void InfraredTimerCallback(void *arg)
    {
        GezipaiBoard *board = static_cast<GezipaiBoard *>(arg);
        board->PollInfraredSensor();
    }

    gpio_num_t GetInfraredSensorGpio(int index)
    {
        return index == 0 ? INFRARED_SENSOR_GPIO : INFRARED_SENSOR_ALT_GPIO;
    }

    bool IsInfraredActive() const
    {
        return infrared_pin_active_[0] || infrared_pin_active_[1];
    }
    ...
    void InitializeInfraredSensor()
    {
        ESP_ERROR_CHECK(gpio_reset_pin(INFRARED_SENSOR_GPIO));
        ESP_ERROR_CHECK(gpio_reset_pin(INFRARED_SENSOR_ALT_GPIO));

        gpio_config_t infrared_config = {
            .pin_bit_mask = (1ULL << INFRARED_SENSOR_GPIO) | (1ULL << INFRARED_SENSOR_ALT_GPIO),
            .mode = GPIO_MODE_INPUT,
            .pull_up_en = GPIO_PULLUP_DISABLE,
            .pull_down_en = GPIO_PULLDOWN_DISABLE,
            .intr_type = GPIO_INTR_DISABLE,
        };
        ESP_ERROR_CHECK(gpio_config(&infrared_config));

        for (int i = 0; i < 2; i++)
        {
            gpio_num_t gpio = GetInfraredSensorGpio(i);
            int level = gpio_get_level(gpio);
            infrared_stable_level_[i] = level;
            infrared_candidate_level_[i] = level;
            infrared_candidate_count_[i] = INFRARED_SENSOR_DEBOUNCE_COUNT;
            infrared_pin_active_[i] = level == INFRARED_SENSOR_ACTIVE_LEVEL;
            ESP_LOGI(TAG, "Infrared sensor GPIO%d initial level: %d", (int)gpio, level);
        }

        esp_timer_create_args_t timer_args = {
            .callback = &InfraredTimerCallback,
            .arg = this,
        ...
        ESP_ERROR_CHECK(esp_timer_create(&timer_args, &infrared_timer_));
        ESP_ERROR_CHECK(esp_timer_start_periodic(infrared_timer_, INFRARED_SENSOR_POLL_INTERVAL_US));
        ESP_LOGI(TAG, "Infrared sensor initialized on GPIO%d/GPIO%d, active level=%d",
                 (int)INFRARED_SENSOR_GPIO, (int)INFRARED_SENSOR_ALT_GPIO, INFRARED_SENSOR_ACTIVE_LEVEL);
    }

    void PollInfraredSensor()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            gpio_num_t gpio = GetInfraredSensorGpio(i);
            int level = gpio_get_level(gpio);
            if (level != infrared_candidate_level_[i])
            {
                infrared_candidate_level_[i] = level;
                infrared_candidate_count_[i] = 1;
                continue;
            }

            if (infrared_candidate_count_[i] < INFRARED_SENSOR_DEBOUNCE_COUNT)
            {
                infrared_candidate_count_[i]++;
                continue;
            }

            if (level != infrared_stable_level_[i])
            {
                infrared_stable_level_[i] = level;
                ESP_LOGI(TAG, "Infrared sensor GPIO%d stable level: %d", (int)gpio, level);
                if (level == INFRARED_SENSOR_ACTIVE_LEVEL)
                {
                    infrared_pin_active_[i] = true;
                }
                else
                {
                    infrared_pin_active_[i] = false;
                }
            }
        }

        bool infrared_active = IsInfraredActive();
        if (infrared_active)
        {
            infrared_last_active_time_ = now;
            if (!infrared_customer_present_)
            {
                infrared_customer_present_ = true;
                infrared_last_notify_time_ = now;
                ESP_LOGI(TAG, "Infrared customer present");
                Application::GetInstance().OnCustomerDetected();
            }
        }

        if (infrared_customer_present_ && infrared_last_active_time_ > 0 &&
            now - infrared_last_active_time_ >= INFRARED_SENSOR_ABSENCE_TIMEOUT_US)
        {
            infrared_customer_present_ = false;
            infrared_last_notify_time_ = 0;
            ESP_LOGI(TAG, "Infrared customer absent");
            Application::GetInstance().OnCustomerGone();
        }
        else if (infrared_customer_present_ &&
                 infrared_active &&
                 now - infrared_last_notify_time_ >= INFRARED_SENSOR_PRESENT_RETRY_US &&
                 Application::GetInstance().GetDeviceState() == kDeviceStateIdle)
        {
            infrared_last_notify_time_ = now;
            ESP_LOGI(TAG, "Infrared customer present retry");
            Application::GetInstance().OnCustomerDetected();
        }
...
public:
    ...
    ~GezipaiBoard()
    {
        if (infrared_timer_ != nullptr)
        {
            esp_timer_stop(infrared_timer_);
            esp_timer_delete(infrared_timer_);
        }
